...
# First build the docker images needed to deploy
	...
	@id -Gn | grep -qw docker || sudo usermod -a -G docker $(whoami) && printf "\033[92m========== You are not in the user group, Ctrl + C now or you will be prompted for sudo to automatically do it ==========\033[0m\n"
